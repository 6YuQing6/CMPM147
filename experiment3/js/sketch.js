...
new p5(function (p) {
  ...
  p.setup = function () {
    ...
    const dungeonCanvas = p.createCanvas(
      tileSize * numRows,
      tileSize * numCols
    );
    ...
    // worldCanvas.parent("canvas-container");
  ...
  function updateAsciiBox() {
    p.select("#asciiBox").value(gridToString(currentGrid));
  }
  ...
  function generateGrid(numCols, numRows) {
    ...
    let numDungeons = p.floor(p.random(3, 6));
    let rooms = [];
    ...
    function generateDungeonRooms() {
      ...
      let roomHeight = p.floor(p.random(2, numRows / 3));
      let roomWidth = p.floor(p.random(4, numCols / 3));
      // start corner (top left)
      let si = p.floor(p.random(margin, numRows - margin * 2));
      let sj = p.floor(p.random(margin, numCols - margin * 2));
      // prevents overlap a little
      while (grid[si] == "." || grid[sj] == ".") {
        si = p.floor(p.random(margin, numRows - margin * 2));
        sj = p.floor(p.random(margin, numCols - margin * 2));
      }
      // end corner (bottom right)
      let ei = p.min(si + roomHeight, numRows - margin);
      let ej = p.min(sj + roomWidth, numCols - margin);
      ...
      const centerRow = p.floor(si + roomHeight / 2);
      const centerCol = p.floor(sj + roomWidth / 2);
    ...
    function carveCorridor(room1, room2) {
      ...
      if (p.random() < 0.5) {
        ...
        for (let c = p.min(c1, c2); c <= p.max(c1, c2); c++) {
          grid[r1][c] = ".";
        }
        for (let r = p.min(r1, r2); r <= p.max(r1, r2); r++) {
          grid[r][c2] = ".";
        }
      } else {
